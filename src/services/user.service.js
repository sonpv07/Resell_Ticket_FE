...
class UserService {
  ...
  static async getTransactionByCustomerId(customerId) {
    try {
      const response = await api.get(
        `/Transaction/get-transaction-buy-package-successful`,
        {
          params: { customerId },
        }
      );
  ...
  static async updatePayOs(customerId, clientId, apiKey, checksumKey) {
    try {
      const response = await api.put(
        `${URL}/PayOS?id=${customerId}&clientId=${clientId}&apiKey=${apiKey}&checksumKey=${checksumKey}`,
        {}
      );

      if (response.status >= 200 && response.status < 300) {
        return returnValue(true, response.data, "Update Payos success");
      } else {
        return returnValue(false, response.data, "Update Payos failed");
      }
    } catch (error) {
      console.error("Error Edit Payos", error);
      return returnValue(false, error, "Update Payos failed");
    }
  }
